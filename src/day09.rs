...
fn parse(input: &str) -> Vec<u32> {
    ...
    let output1 = input
        .trim().chars().map(|c| c.to_digit(10).unwrap()).collect::<Vec<u32>>();
...
fn part2(input: &str) -> u64 {
    ...
    for (idx, file_len) in x.iter().enumerate() {
        if idx %2 == 0 {
            let start = checksum_vec.len();
            for _ in 0..*file_len {
                checksum_vec.push((idx/2) as isize);
            }
            let end = checksum_vec.len();
            files.push(File { start: start, end: end, id: (idx/2) as isize });
            ...
            let end = checksum_vec.len();
            freespace.push(File { start: start, end: end, id: -1 });
        }

    }
    let mut compacted_files: Vec<File> = vec![];
    while !files.is_empty() {
        let next_file = files.pop().unwrap();
        match find_next_free(next_file.end - next_file.start, &freespace) {
            None => {
            ...
            Some(idx) => {
                ...
                if new_start > next_file.start {
                    compacted_files.push(next_file);
                    continue;
                }
                let new_file = File{start: new_start, end: new_end, id: next_file.id};
...
fn find_next_free(file_len: usize, freespace_vec: &Vec<File>) -> Option<usize>{
...
fn compact_around(idx: usize, freespace_vec: &mut Vec<File>) {
    if idx < freespace_vec.len() - 1 {
        if freespace_vec[idx+1].start == freespace_vec[idx].end {
            freespace_vec[idx].end = freespace_vec[idx+1].end;
            freespace_vec.remove(idx+1);
        }
    }
    if idx > 0 {
        if freespace_vec[idx-1].end == freespace_vec[idx].start {
