...
fn part1(input: &str) -> u64 {
    ...
    let opcodes: Vec<Opcode> = instructions.chunks(2).map(|x| 
        match x[0] {
            0 => Opcode::Adv(x[1]),
            1 => Opcode::Bxl(x[1]),
            2 => Opcode::Bst(x[1]),
    ...
    loop {
        match opcodes[cur_pointer] {
            Opcode::Adv(v) => {registers[0] >>= combo(v, &registers);},
            Opcode::Bxl(v) => {registers[1] = registers[1]^v;},
            Opcode::Bst(v) => {registers[1] = combo(v, &registers)%8;},
            Opcode::Jnz(v) => {if registers[0] != 0 {
...
fn recurse(depth: usize, target: &Vec<u64>, so_far: u64, opcodes: &Vec<Opcode>) -> Option<u64> {
    ...
    for trial in 8*so_far..8*so_far + 8 {
        println!("{:?}", run(trial, opcodes));
        println!("{:?}", &target[depth-1..]);
